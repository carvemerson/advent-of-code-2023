...
function getLeftDigit($string)
{
    ...
    foreach($numbers as $key => $value) {
        $p = strpos($string, $key);
        if ($p !== false && $p < $position) {
            $position = $p;
            $digit = $value;
        }

        $p = strpos($string, $value);
        if ($p !== false && $p < $position) {
            $position = $p;
            $digit = $value;
        }
...
function getRightDigit($string)
{
    ...
    foreach($numbers as $key => $value) {
        $p = strrpos($string, $key);
        if ($p !== false && $p > $position) {
            $position = $p;
            $digit = $value;
        }

        $p = strrpos($string, $value);
        if ($p !== false && $p > $position) {
            $position = $p;
            $digit = $value;
        }
...
foreach($input as $string) {
    $int = (int) (getLeftDigit($string) . getRightDigit($string));
    $result += $int;
}
